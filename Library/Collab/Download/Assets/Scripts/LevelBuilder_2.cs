...
public class LevelBuilder_2 : MonoBehaviour
{
	...
	IEnumerator GenerateLevel()
	{
		...
		for (int i = 0; i < iterations; i++)
		{
			...
			PlaceRoom();
			Debug.Log("[LV1] Place random room from list");
			yield return interval;
		}

		// Place monster room
		PlaceEndRoom();
		Debug.Log("[LV1] Place End Room");
		yield return interval;

		PlaceMonsterRoom();
		Debug.Log("[LV1] Place Monster Room");
		yield return interval;

		// Level generation finished
		Debug.Log("[LV1] Level generation finished");
		GetSpawnPoint();

		gameManager.tpToArena1FromLevel1 = GameObject.Find("tpToArena1FromLevel1").transform;
	...
	{
		Debug.Log("[LV1] Positionning Enemies");
		float rdn;
		foreach (Vector3 pos in poz)
		{
			rdn = Random.Range(0, 99);
			if (rdn > 20) gameManager.SpawnEnemy("Prefabs/Chicken", pos);
			else gameManager.SpawnEnemy("Prefabs/Red Panda", pos);
		}
		gameManager.BuildLevel2();
	}
	...
	void PlaceStartRoom()
	{
		...
		// Position room
		startRoom.transform.position = new Vector3(0.0f, 20.0f, 0.0f);
	...
	void PlaceRoom()
	{
		...
		foreach (Doorway availableDoorway in allAvailableDoorways)
		{
			...
			foreach (Doorway currentDoorway in currentRoomDoorways)
			{
				...
				Debug.Log("[LV1] Checking overlap");
	...
	bool CheckRoomOverlap(Room room)
	{
		...
		Vector3 pt7 = new Vector3(-5, 20, 7);
		Vector3 pt8 = new Vector3(-5, 20, -7);
		Vector3 pt9 = new Vector3(15, 20, -4);
		Vector3 pt10 = new Vector3(15, 20, 4);
		Vector3 pt11 = new Vector3(-4, 20, 2);
		Vector3 pt12 = new Vector3(-11, 20, 1);
		...
		foreach (Room r in placedRooms)
		{
			...
			{
				Debug.LogError("[LV1] Overlap Detected");
				return true;
			}
		}
	...
	void PlaceMonsterRoom()
	{
		...
		foreach (Doorway availableDoorway in allAvailableDoorways)
		{
			...
			Debug.Log("[LV1] Monster Room placed");
			roomPlaced = true;

			placedRooms.Add(monsterRoom);
	...
	{
		Debug.LogError("[LV1] Reset level generator");
