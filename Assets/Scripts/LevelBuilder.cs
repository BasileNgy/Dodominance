...
public class LevelBuilder : MonoBehaviour
{
	...
	IEnumerator GenerateLevel()
	{
		...
		for (int i = 0; i < iterations; i++)
		{
			...
			PlaceRoom();
			Debug.Log("["+name+"] Place random room from list");
			yield return interval;
		}

		// Place end room
		PlaceEndRoom();
		Debug.Log("[" + name + "] Place End Room");
		yield return interval;


		// Level generation finished
		Debug.Log("[" + name + "] Level generation finished");
		GetSpawnPoint();
	...
	{
		Debug.Log("[" + name + "] Positionning Enemies");
		float rdn;
		foreach (Vector3 pos in poz)
		{
            ...
            {
				if (rdn > 15) gameManager.SpawnEnemy("Prefabs/Red Panda", pos);
				else gameManager.SpawnEnemy("Prefabs/Panda", pos);
			}
			
		}

		gameManager.MapGenerated(this);
	}
	...
	bool CheckRoomOverlap(Room room)
	{
		...
		Vector3 pt7 = new Vector3(-5, yOffSet, 7);
		Vector3 pt8 = new Vector3(-5, yOffSet, -7);
		Vector3 pt9 = new Vector3(15, yOffSet, -4);
		Vector3 pt10 = new Vector3(15, yOffSet, 4);
		Vector3 pt11 = new Vector3(-4, yOffSet, 2);
		Vector3 pt12 = new Vector3(-11, yOffSet, 1);
		...
		foreach (Room r in placedRooms)
		{
			...
			{
				Debug.LogError("[" + name + "] Overlap Detected");
				return true;
			}
		}
	...
	{
		Debug.LogError("[" + name + "] Reset level generator");
