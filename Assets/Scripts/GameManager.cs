...
public class GameManager : MonoBehaviour
{
    ...
    private LevelBuilder level1, level2;
    ...
    IEnumerator CheckIfIntroFinished()
    {
        ...
        while (!donePlaying)
        {
            ...
            {
                introCinematique.Stop();
                donePlaying = true;
            }
            if (introCinematique.frame > 0 && (introCinematique.isPlaying == false))
            {
                donePlaying = true;
            }
            yield return null;
        }
        introCinematique.gameObject.SetActive(false);
        canvasManager.ToggleCanvas(CanvasType.MainMenuCanvas);
    }
    ...
    public void BuildLevel1()
    {
        ...
        level1.gameManager = me;
        level1.NewMap();

    }
    public void MapGenerated(LevelBuilder level)
    {
        if (level.name.Equals("Level1"))
        {
            BuildLevel2();
        } else if (level.name.Equals("Level2"))
        {
            RestartGameStats();
        }
    }
